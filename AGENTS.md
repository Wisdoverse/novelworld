...
The root launchers copy `.env.example` to `.env`. On a fresh interactive
server install they guide the L0 PostgreSQL user/database values, generate the
database password, commit `BOOTSTRAP_L0_COMPLETE=true` last, and restart once
before starting Docker. Valid preseeded environments migrate without prompting;
unconfigured non-interactive launches fail closed. Bootstrap/runtime surfaces:

- `BOOTSTRAP_L0_COMPLETE` — launcher-owned L0 commit marker; never set it
  without valid `POSTGRES_USER`, `POSTGRES_DB`, and `POSTGRES_PASSWORD`
