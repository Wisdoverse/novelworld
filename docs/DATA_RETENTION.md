...
|---|---|---|
| Uploaded TXT, EPUB, or PDF bytes | With `S3_ENABLED=true`, stored privately under a server-generated key for the lifetime of the shared canonical novel and read back during import recovery to replay chapter splitting before any provider work. With S3 disabled, held only while the request extracts text. | Canonical-novel deletion atomically queues the object key in PostgreSQL; novel-service deletes it asynchronously and retries with bounded backoff until S3 acknowledges deletion. Shelf removal and uploader-account deletion retain the shared asset. |
| Extracted source text and lore chunks | PostgreSQL `chapters` and `chapter_chunks`, for the lifetime of the shared canonical novel. | Canonical-novel deletion. Shelf removal and account deletion retain shared content. |
| Import job metadata | PostgreSQL `novel_import_jobs` stores only stage, status, attempt, lease, and failure code for the lifetime of the shared canonical novel; it contains no source or model output. | Canonical-novel deletion cascades the row. It is internal operational state and is excluded from account export. |
| Characters, relationships, and canonical models | PostgreSQL, for the lifetime of the shared canonical novel. Avatar bytes are not copied into NovelWorld; only provider-returned URL metadata is stored. | Canonical-novel deletion removes the rows and URL metadata. Shelf removal and account deletion retain them. |
| Confirmed series definitions and source-template snapshots | Novel-owned PostgreSQL rows scoped to the reader; snapshots contain fixed rules, numeric values, and real source provenance, not novel text. | Removing a shelf link deletes only membership. Definitions and frozen bindings remain until account deletion. Deleting a canonical source does not cascade into a snapshot used by another book. Account export includes definitions and memberships. |
| Chat turns and messages | PostgreSQL, scoped by account and novel. PostgreSQL is authoritative. | Account or canonical-novel deletion. Shelf removal retains durable history for explicit re-attachment. |
