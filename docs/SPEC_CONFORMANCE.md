...
| §12.4.1 — erasure records atomic with every deletion path, minimal UUID-only fields, per-novel records under account cascade, cascade-surviving | Verified | H1 | E9; `AFTER DELETE` row triggers on `users` and `novels` write inside the deleting transaction, the journal has no foreign keys so a cascade cannot remove its own evidence, and drill B exercises the direct and cascade paths. The identifying payload is the subject type and UUIDs; `erased_at`, `had_source` and `source_requeued_at` are operational bookkeeping for replay, carry nothing derived from content, profile or credentials, and the column set is asserted directly |
| §12.4.2 — migration-path replay before services start, idempotent across deployments, removes any subject row matching an erasure record | Verified | H1 | E9; replay runs in the migration container the services wait on, and both the integration tests and drill B replay it twice with no further effect |
| §12.4.2 — retained-source object key re-queued exactly once per record within a database lineage, tracked by durable per-record bookkeeping (at most one repeat per restore) | Verified | H1 | E9; the record carries the `had_source` fact the authoritative delete observed, every erasure source transports it, and only a delete can raise it, so replay reconstructs `source-files/{user_id}/{novel_id}` for a subject row this database never held — proven in a database with no other retained-source evidence — while a novel that never held one is never enqueued speculatively. Bookkeeping is stamped only on an actual re-queue. A restored dump keeps its stamps rather than resetting them: stamp and outbox row are written in one transaction that a single-snapshot dump preserves, so those records repeat zero re-queues; a record arriving from a foreign erasure source enters unstamped, because its outbox row is in another artifact, and re-queues once — the clause's at-most-one allowance. The only excluded deletions are those predating this migration, which it declares unknowable and does not backfill |
| §12.4.3 — artifacts embed same-snapshot erasure exports with covered-through timestamps; restore stops writes, replays the union of sources, aborts on conflicting records, refuses non-empty residual windows except through attest-or-erase (every account not already covered by a collected record decided retain-with-listed-novels or erase; erasure records written and replayed for erased accounts and unlisted novels before services start; decisions durably recorded with subject, decision, both window bounds, artifact digest inventory, operator identity, and timestamp), never contains an undecided subject or serves one covered by any erasure record, rotates the JWT secret and deletes all persisted refresh tokens after verification and before services start, clears runtime configuration on final-account removal, and requires designating a retained administrator when decisions would leave none | Verified | H1 | E9; the sidecar export is cut out of the single `pg_dump` stream, so it cannot diverge from the dump, and its covered-through timestamp is read immediately before that snapshot opens — never the archive-write time, and conservatively early, so a window derived from it is a superset. The window is empty only when the live database shares identity with the artifact — a live account the artifact holds, or a live erasure record whose subject the artifact holds — so a replacement host that has been used at all, even to create and delete an account of its own, still faces the gate; an artifact taken before first-run setup intersects nothing and always faces it. The restore aborts when sources disagree on a deletion fact (subject type, subject, owning user, deletion time), while the retained-source marker merges monotonically upward because a false-then-true succession is a bookkeeping upgrade, not a disputed fact. Subjects covered by a collected record are pre-decided facts: they are excluded from the decisions the operator must supply and from the prompt's novel inventory, and no decision may retain or designate them. Every manifest and decision value reaching a SQL literal is shape-checked whole-value at the parse boundary and, for timestamps, validated for calendar truth by the server that stores them; a residual window that ends before it starts aborts, and the column enforces the same ordering. The recorded inventory names only digests this run verified, labelled by what they cover. Drills B and C cover the empty and non-empty windows, undecided, partial, override and erase-all decisions, the administrator invariant and its positive designation, conflicting sources, tampered and multiline metadata, an unrelated live lineage, every recorded decision field, and the rejection of a pre-restore access token and refresh token |
| §12.4.4 — scripted, encrypted, integrity-verified artifacts and fail-closed restore on corrupt or unverifiable input | Verified | H1 | E9; AES-256-CBC with PBKDF2 at 200 000 iterations, a SHA-256 manifest verified before any data change, and drill negatives for a corrupted artifact and a wrong key. The ≤ 30 minute RTO scale rehearsal is not part of this evidence: [`scale_rehearsal.sh`](../infra/backup/scale_rehearsal.sh) is tooling only, never runs in CI, and its recorded run remains separate release evidence |
