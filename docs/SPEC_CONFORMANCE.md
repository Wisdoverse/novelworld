...
| §12.4.1 — erasure records atomic with every deletion path, minimal UUID-only fields, per-novel records under account cascade, cascade-surviving | Verified | H1 | E9; `AFTER DELETE` row triggers on `users` and `novels`, a foreign-key-free journal, and drill B's two deletion paths |
