...
| E7 | Browser contract: [`client.ts`](../frontend/src/shared/api/client.ts), [`client.test.ts`](../frontend/src/shared/api/client.test.ts), [`narrative api tests`](../frontend/src/entities/narrative/api.test.ts), [`ReaderPage.test.tsx`](../frontend/src/pages/reader/ui/ReaderPage.test.tsx), [`BranchChoice.test.tsx`](../frontend/src/widgets/branch-choice/ui/BranchChoice.test.tsx), [`ChatPanel.test.tsx`](../frontend/src/widgets/chat-panel/ui/ChatPanel.test.tsx), [`globals.css`](../frontend/src/app/styles/globals.css) |
...
| §7.2 — exact same-index choice replay, different-index conflict, and atomic choice/world/chapter persistence while source remains immutable | Verified | H4 | E1, E4, E7; the natural `(user_id, node_id)` key is checked before generation and again after the competing commit winner is read; browser recovery reloads the committed world/chapter state after a conflict |
...
| §7.6 — durable original player, validated world turns, commit-before-prose, and ordered recent-turn continuation | Verified | H4 | E4, E7; the current single-timeline prompt receives the last four committed actions and bounded narrative endings, thread actions require an open target, v1 committed transitions remain replayable, and ambiguous delivery/lease-loss outcomes retain the original idempotency key |
| §7.6 — branch, chapter, chat, journal, and world-turn consumers form one live-quality causal system | Intended gap | H4 | E0, E4; structural state sharing and recent-turn continuation do not yet qualify a live long trajectory across every consumer |
