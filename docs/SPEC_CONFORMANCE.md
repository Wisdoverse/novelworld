...
| E11 | Supply-chain gates: [`audit.toml`](../.cargo/audit.toml), [`Cargo.lock`](../Cargo.lock), the frontend [`pnpm-lock.yaml`](../frontend/pnpm-lock.yaml), [`.gitleaks.toml`](../.gitleaks.toml), [`deny.toml`](../deny.toml), [`scan-images.sh`](../infra/security/scan-images.sh), [`generate-sboms.sh`](../infra/security/generate-sboms.sh), the direct `cargo-audit`, production `pnpm audit`, `gitleaks`, `cargo-deny`, and trivy CI steps, and the Dependency Policy in [`SECURITY.md`](../SECURITY.md) |
...
| §15 — known-vulnerability dependency gate | Verified | H2 | E11; the AWS SDK uses its current `default-https-client` path, so the redundant legacy `rustls` feature no longer pulls vulnerable rustls-webpki 0.101.7 or h2 0.3.27; a local cargo-audit 0.22.2 run against the current `Cargo.lock` is clean with no vulnerability ignores in `.cargo/audit.toml`; jsonwebtoken uses its `aws_lc_rs` backend so the rsa crate is not in the tree; CI runs `cargo-audit` directly against the committed `Cargo.lock`, and the frontend job runs `pnpm audit --prod --audit-level high` against the frozen lockfile, so a new high/critical Rust or shipped-browser dependency advisory fails the build. Informational Rust warnings (ttf-parser unmaintained, lru unsound pop) remain non-failing and re-reviewed on chain updates; development-only frontend tooling is outside this production-dependency gate; deploy-time SBOM verification, provenance/attestation, and signature gates remain open H2 items |
