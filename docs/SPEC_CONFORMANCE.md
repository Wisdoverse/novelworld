...
| §4.1.11/§5.1/§12.3 — import-created progress metadata and schema-aware replay of migration 0002 | Verified | H1 | Read-only aggregate inventory found one progress row with no effective chapter within the advertised count; migration 0002 preserves non-ready bootstrap progress, fails for ready or missing-status novels without an effective chapter, and normalizes only against existing chapters. E1 local PostgreSQL regression verifies replay and row preservation; required CI evidence is tracked by Issue #424. |
| §12.3 — migration 0019 first-adoption uploader-shelf backfill and replay | Verified | H1 | The migration records pre-existing `user_novels` presence transaction-locally under the `novels` lock; only a previously absent relation enables backfill. E1 local PostgreSQL regression: `legacy_schema_upgrade_is_lossless_and_replay_safe` verifies initial backfill; `full_migration_replay_preserves_incomplete_progress_and_terminal_imports` verifies detached and empty shelves during repeated full replay; required CI evidence is tracked by Issue #424. |
...
| §6.1 — complete persona, world, progress, identity, and deviation prompt | Intended gap | H3 | E0, E3; at exact full-book progress, whole-novel aliases, role, description, personality, background, speaking style, the exact non-empty, at-most-2,000-character world summary, and a bounded requested-character projection of source-validated immutable Canon v1 goals and relationships enter the provider prompt only as bounded JSON-quoted data. Canon grounding is loaded only for an acquired turn through authenticated Novel HTTP; Novel rechecks owned exact-full progress after Canon I/O, Agent strictly validates the response, and later committed per-reader world state takes precedence. Partial, rewound, missing, malformed, wrong-scope, or marker-mismatched context fails before provider work. Syntactic inertness is not a semantic injection guarantee; representative live prompt quality remains a gap |
| §6.1/§15 — progress-bounded public persona and Agent admission | Verified | H3 | E2, E3, E7; before `Ready && current == total`, list/detail JSON contains only `id`, `novel_id`, source-proven canonical `name`, and `first_appearance_chapter`; alias-only evidence is rejected, and new chat fails before claim/provider work because no whole-novel summary is authorized. Full persona and `world_summary` share the exact `persona_source_chapter_high_water`; `system_prompt` is absent in every public state, and all list/detail responses are `private, no-store`. The public relationship endpoint returns an empty list until the owned novel is Ready at exact full progress, then preserves the existing graph shape only if a final persisted-progress recheck after relationship I/O still matches; all relationship responses are also `private, no-store`. For an acquired turn at that same exact-full boundary, Agent obtains only the requested character's bounded goals and relationships from immutable Canon v1, rejects malformed or mis-scoped responses, strips routing IDs from the prompt projection, and makes committed per-reader state authoritative over source history. Agent independently requires a non-empty summary of at most 2,000 characters and a marker equal to its final persisted progress snapshot; old or malformed Novel responses fail closed. Reclaimed turns require the latest validated marker to equal the persisted marker before prompt/provider work. Pre-contract unmarked turns and derived Mid/Long rows remain exportable/deletable but are excluded from online history, prompt composition, source-window summaries, direct/semantic retrieval, and completed replay. The browser rebuilds an explicit allowlist and re-derives selected objects by id after query changes as a race fence; it is not an authorization boundary. E12 predates this prompt contract; progressive public relationship publication, export-wide spoiler safety, H1 extraction quality, live goal/relationship/world-summary quality, and H4 release qualification remain open |
...
| §6.4 — bounded four-layer prompt composition, explicit character visibility, current-progress recheck, and total prompt budget | Intended gap | H3, H4 | E0, E3, E4; deterministic tests cover source bounds, de-duplication, fail-closed aggregate size, and exact bounded world-summary carriage in streaming and non-streaming prompts. They also cover selection of the latest four directly targeted `converse`/`ally`/`oppose` actions from a bounded 100-turn journal scan, independent Narrative/Agent action kind+target rejection, and producer/Agent-consumer event exclusion unless its actor list names the character. The provider prompt is a tested allowlist: choices, unscoped threads, player name/location, routing UUIDs, and technical metadata are excluded; the progress-authorized world summary plus character-specific state/actions/events remain. All derived context is omitted when its source high-water is absent or exceeds rewound progress. Inferred visibility and visibility beyond that scan are unsupported outside `h4-journey-qualification-v1`, not H4-v1 release blockers; judged live relevance and lifecycle evidence remain gaps |
...
| §10.1 — fixed regional API/plan presets and credential isolation | Verified | H2 | `RuntimeLlmConfig` endpoint/key-isolation tests, shared transport request tests, Settings unit tests and `llm-providers.spec.ts` establish offline configuration and browser behavior. Internal provider metadata preserves plan labels; new model IDs do not establish live provider qualification or subscription eligibility. See [provider limits](./LLM_PROVIDERS.md) |
...
| §12.3 — versioned migrations, replay safety, initial application, and schema-version barriers | Verified | H1, H5 | E1, E8; migrations 0021, 0024, 0025, and 0030 are explicit transactional, replay-safe application-semantic barriers. PostgreSQL 18 contracts prove lossless preservation of legacy rows, new-protocol replay, exact readiness constraints, and fail-closed detection of weakened constraints. Release tooling stops old Narrative before exposing the candidate client, pins the gate to a retryable world-action `5xx`, then stops old Novel and drains Agent before writing the durable exact-target marker and migrating. Every marked transition rolls that target forward; promoted state is synced before marker removal, and removal is synced again. Adoption requires all four barriers; upgrade, marked restore, and rollback reject a backwards crossing, so the supported managed Docker path cannot revive a pre-0024 Agent or Novel or a pre-0025 Agent or an old Novel writer after 0030 without a separately approved compatibility procedure. Interrupted upgrades preserve old current as previous, initial adoption supports a missing candidate without inventing previous, and mismatched candidates fail closed. Local launchers stop the Compose stack without volume deletion before migration and restart; their ordering is self-tested. The desktop migration list embeds 0021, 0024, 0025, and 0030 and applies them after stopping pg0/validating free ports but before spawning services; experimental desktop archives are forward-migration-only because an older binary cannot enforce a newer barrier. The 0030 preservation, cross-version budget/lease and legacy replay contracts pass locally; exact-head CI, merge and deployment evidence is tracked in [issue #422](https://github.com/Wisdoverse/novelworld/issues/422) |
...
| §13.1 — FSD import boundaries | Verified | H4 | E7; [`check-fsd.mjs`](../frontend/scripts/check-fsd.mjs), required via `pnpm lint:fsd`, enforces downward imports, same-layer slice isolation and sliced-layer root public APIs across every source TypeScript module, including tests and mocks. Its negative self-tests cover these boundaries; only the documented same-slice and non-sliced composition exceptions apply. See the [canonical FSD contract](../AGENTS.md#architecture-constraints). No legacy allowlist; static enforcement does not prove semantic ownership or runtime behavior |
| §13.2 — shared catalog shows every ready novel, limits returned fields, and preserves existing shelves on repeated attach | Verified | H1, H4 | [`novel_pg_repo.rs`](../services/novel-service/src/infrastructure/persistence/novel_pg_repo.rs), [`repository_contracts.rs`](../tests/integration/tests/repository_contracts.rs), E7; the authenticated catalog keeps ready novels visible after attachment, excludes failed imports and import diagnostics, and the Novel serializer omits uploader identity and source locators. The shelf UI disables the repeated action; the repository leaves existing progress and deviation mode unchanged on a repeated request. This is structural evidence, not a live multi-reader journey. |
...
| §14.1 — structured logs, trace context, and request outcome fields | Verified | H2, H5 | E8 plus the required production log-contract checker: all five services carry service + trace_id, and Gateway propagates a bounded X-Trace-Id; the checker validates JSON shape, end-to-end propagation, completion method, route template, status, elapsed_ms, and status/level mapping. Gateway unit coverage proves that error-only filtering retains service/trace context and omits raw URI values; the LLM client unit test uses a query-bearing failed endpoint to prove retry WARN logs omit its URL. |
| §14.2 — liveness/readiness separation and Gateway aggregation | Verified | H2, H5 | E8 and Production Compose Smoke |
| §14.3 — private metrics route, bounded labels, key-scoped usage visibility, and no private content | Verified | H2, H5 | E8; embedding transport uses its own bounded `novelworld_embedding_*` namespace and cannot be misclassified as a chat operation in the closed `llm-observability-v1` budget contract. Billable chat series use only a one-way actual-key fingerprint, user-service filters the current platform/personal key server-side, and readers using the platform fallback cannot query its aggregate. The offline [quote snapshot and estimate tests](./LLM_PRICING.md) add native-currency token estimates/ranges and separate subscription quotes without changing metric labels or frozen budgets; historical account bills, remaining quotas, and representative deployment collection remain unqualified |
...
| §15 — SBOM generation | Verified | H2 | E11; the current release pipeline generates one CycloneDX 1.7 SBOM per application image with the pinned trivy release and ships them with the release artifact, bound to the recorded image digest via `sboms/digests.txt`; `infra/security/generate-sboms.sh` is the local form. The earlier local and v0.1.0 release records report CycloneDX 1.6 and remain historical evidence, not the current producer contract; deploy-time SBOM admission remains open, while release-file provenance implementation and its exact source, run, and native acceptance evidence are owned by [Issue #274](https://github.com/Wisdoverse/novelworld/issues/274) |
...
- Self-mode prompt context currently retrieves two independently bounded permanent
  ...
  not backfilled, and rollback to an older Agent suspends this guarantee.
  See [ADR 0005](./adr/0005-durable-chat-summary-windows.md) for adoption,
  eligibility, deadlines and evidence limits. It attempts best-effort mid-to-long
  promotion after that one Mid commit, and gives every committed open-world turn
...
- World-action recovery persists one bounded, schema-validated `(action,
  ...
  This survives a reload/remount in that tab session. If storage is blocked,
  the stable server view returns only the same-session, semantically valid
  `in_progress` action/key for explicit replay. This is not cross-device
  persistence or a server background-reconciliation mechanism, and an expired
  lease may still be superseded under the existing concurrency contract.
