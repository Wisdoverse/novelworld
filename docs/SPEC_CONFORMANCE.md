...
| §13.4 — declared visual tokens and reading typography | Verified | H4 | E7; axe-core 4.13 scans run in required CI on the critical journey (login, shelf, reader, characters, settings, setup, player entry, world dashboard, world action form) with zero AA violations under the jsdom rule set (color-contrast disabled: jsdom cannot compute real styles — the real contrast fixes are recorded on issue #165 and enforced by a browser-based gate as the remaining gap). Component-level scans skip page-level WCAG A rules (document-title, html-has-lang, landmark structure) and the color-contrast rule is disabled in jsdom (real contrast fixes are recorded and enforced by a browser-based gate); this is still not WCAG qualification: page-level automation, the browser contrast gate, and manual keyboard/screen-reader/mobile review remain open (H4) |
