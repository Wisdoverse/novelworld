...
| Four-layer memory provides cross-session continuity | Mid-term summaries exist and now promote into the long-term layer with 1536-dim embeddings on the production projection path ([PR #145](https://github.com/Wisdoverse/novelworld/pull/145)); promotion is unselective (every mid summary), generation failure or non-1536 provider vectors skip it, evidence is deterministic tests — not live semantic quality; permanent-memory writing is still not connected to the journey | H0 decides the contract; H3 proves the outcome |
