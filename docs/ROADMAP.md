...
| Consecutive player actions remain causally connected | The authoritative `world_turns` journal is a per-reader total order. The current world-turn prompt receives a bounded tail of committed actions and narrative endings, thread actions validate their open target, and ambiguous outcomes retain the original logical-turn key. Cross-consumer and live long-trajectory coherence remain unqualified; no graph database is required for this single-writer timeline | H4 |
