...
| Novel-specific D20 advanced mode | Optional, unqualified preview | Narrative remains the default. An explicit request lazily generates one source-bound attribute/DC template per canonical novel model and shares it across authorized readers; the exact immutable template remains hidden until every cited chapter is unlocked, with content-free `422 game_rules_unavailable_at_progress`. Advanced turns retain server-owned hard validation and dice authority. When paired Laya (Jev) settings are present, one bounded semantic classification may select impossible, automatic success, or a difficulty band; absent configuration, uncertainty, and errors fall back to the template DC. Classification and resolution are fenced and frozen in the existing world-turn record before DeepSeek prose; replay reuses the frozen result. No die is shown as evidence for no-check outcomes, and impossible actions follow failed-check mutation clearing. Context is limited to the reviewed allowlist and size cap; it excludes novel full text, scope UUIDs, history, future-event lists, dice, and results. The classifier probability is uncalibrated. Full D&D rules, arbitrary free-text guarantees, representative live semantic quality, paid comparison, and human semantic-quality approval are not claimed. See [ADR 0009](./adr/0009-bounded-laya-d20-adjudication.md) and the [rules plan](./ADVANCED_RULES_PLAN.md) | H4 |
