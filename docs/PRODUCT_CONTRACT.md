...
|---|---|---|
| Deployment | Operator-controlled, private, single-node Docker Compose preview on localhost, or behind an operator-managed encrypted private-network/TLS boundary | Production Compose and CI exercise one instance of each service. Internet exposure is unsupported; default Nginx has no TLS termination (the headers below are not a substitute for it); it serves baseline security headers (nosniff, clickjacking denial, same-origin referrer policy); gateway CORS is restricted to the documented preview origins (`CORS_ORIGINS`), and downstream routers stay permissive but are not browser-reachable in this envelope. The supported schema-transition sequence stops old Narrative before exposing candidate client assets, verifies world actions are retryably fail-stopped, then stops old Novel and Agent processes before migration. The required barriers are 0021, 0024, 0025, and 0030. Migration 0030 additionally requires both old Novel and Narrative writers stopped and drained because the old Novel writer is incompatible; compatible versions start together. Existing v1 profiles/sessions remain on v1; zero-downtime world actions are not claimed. A durable exact-manifest marker is synced immediately before migration; every marked transition rolls that exact target forward, the target tempfile and any former current renamed as previous are synced before current replacement, promoted state is synced before marker removal, and removal is synced again. Adoption requires all four barriers, while upgrade, marked restore, and rollback reject a backwards crossing. The one-time Redis 7-to-8 re-pin is supported only as a documented cold re-adoption of a no-user Unix preview: ingress and writers stop, the non-authoritative Redis projection and deletion-tombstone volume is deliberately discarded, PostgreSQL is retained, and the archived Redis 7 manifest ceases to be recovery authority before Redis 8 starts. CI proves fresh-volume Redis 8 compatibility and exact image identity, not that manual cutover sequence, host-crash recovery, a Windows in-place transition, or preservation of Redis projections. The drill pins application-schema ordering and simulated process recovery; real Linux host power loss and live registry/image health remain unqualified. An installation whose active release tooling predates these controls needs a control-only release before the migration release. |
| Platforms | Linux shell and Windows 10/11 launchers with Docker Compose | Local launchers stop the old project without removing named volumes before rebuild/start, forcing the one-shot migration container to rerun with old writers quiesced; the exact down-before-up order is pinned by `start.ps1 -Check`. Desktop startup stops pg0, refuses occupied service ports, embeds migrations through 0031, and applies them before spawning services. The existing 0030 writer shutdown barrier is unchanged; 0031 is additive. Series journeys require matching Novel, Narrative, and frontend versions; older peers reject the series prompt. After series-bound state exists, returning to an application that cannot read it is unsupported. Experimental desktop archives support forward migration only. Linux paths run in CI; portable Windows/Linux/macOS GitHub Release assets remain experimental until their version-matched artifacts pass the required journey and signing gates. This is not a qualified OS/browser matrix. |
...
| Key usage and cost visibility | Current-key Prometheus token increments with offline official-price snapshots, native CNY/USD subtotals, and separately identified subscription quotes | [Pricing limits](./LLM_PRICING.md) apply: missing request time/context/mode yields a current-price range; unverified or retired prices remain unpriced. Missing cache metadata and fees outside the token categories prevent historical bill reconstruction. Subscription quotes neither establish backend-use eligibility nor infer the bought tier, paid bill, or remaining quota. |
| Architecture | Static Cloud Native, DDD, and microservice source boundaries for the private `single-node-v1` topology | The versioned Rust architecture gate checks five runtime packages, reviewed local helpers and transport capabilities, layer/package edges, HTTP adapter placement, analyzable SQL and routine ownership, canonical relation/routine/view/trigger inventory, declared FK/trigger debt, and static process hooks. It does not prove database grants or isolation: one role/schema remains shared, with 21 cross-owner cascading FKs, two lifecycle-trigger accesses, five cross-owner trigger/routine bindings, ten exact historical migration audit hashes, and narrow readiness dependencies. It also does not exhaust unknown external crates or generated code, or qualify drain/timeout completeness, recovery, monitoring/alerting, replicas, horizontal scaling, or public-cloud operation. |
...
| One-click and bounded batch import | Accepted and structurally verified inside the input limits | A request accepts one novel, or up to five uploaded files whose source bytes total at most 40 MiB. Batch acceptance commits every independent Novel, shelf/progress row, chapters or retained-source boundary, and pending durable job in one PostgreSQL transaction; it attempts to claim one job only if parsing capacity is available and leaves pending work to the existing leased recovery path. Parsing admission does not block upload acceptance. The browser selects up to 50 files and sends bounded requests sequentially; the full selection is not atomic, and unknown batch outcomes require shelf inspection before resubmission. Import POST responses report `accepted`; status/list reads own parsing progress. No batch aggregate, pause, cancel, or per-file metadata editor is claimed. Single-import fenced leases resume `source`/`chapters`/`enriched` work; live kill drills at the `chapters` and `enriched` boundaries pass in CI, and the S3 `source`-boundary drill passes in required CI ([PR #135](https://github.com/Wisdoverse/novelworld/pull/135)); cross-attempt provider calls stay inside `import-provider-budget-v1` (3-claim ceiling, terminal `budget_exhausted`); live semantic quality remains unqualified | H1 |
| Shared parsed novels with private user worlds | Structurally verified | The authenticated shared catalog lists every ready canonical novel, including books already on the reader's shelf; pending, parsing, and failed imports stay out. Catalog responses exclude uploader identity, source storage locators, import diagnostics, and reader-private state. Already attached books are labeled and cannot be added again. A different reader can attach the same canonical novel without uploading or parsing it again; shelf authorization, reading progress, identity, deviation mode, choices, chat/memory, and world state remain user-scoped. Repeating an attachment preserves existing progress and deviation mode. Deleting the uploader preserves the canonical asset for other shelves. Automatic same-content detection is not claimed; reuse is an explicit catalog action. | H1, H4 |
| Canonical world model and relationship graph | Structurally verified | The public relationship graph is withheld as an empty list until the owned novel is `Ready` and persisted progress is exactly complete; an initially complete read rechecks persisted progress after relationship I/O and fails closed on rewind or drift. Every success and error is `private, no-store`. The full response keeps the existing graph shape because per-relationship chapter provenance is not persisted. At the same exact-full boundary, Agent consumes a bounded requested-character projection of source-validated Canon v1 goals and relationships through authenticated Novel HTTP, with final progress recheck, strict scope validation, untrusted JSON quoting, and committed per-reader state precedence. Source coverage exists in deterministic tests; gradual public relationship publication and representative live quality are not qualified | H1, H3 |
| Character personality and authentic voice | Intended gap | Extraction remains whole-novel, but persona and world-summary publication and Agent consumption are progress-bounded: before `Ready && current == total`, character list/detail reads expose only a source-proven canonical name with `id`, `novel_id`, and first appearance, and new chat fails before claim/provider work because no whole-novel summary is authorized. At exact full progress, the allowlisted persona and non-empty, at-most-2,000-character `world_summary` share an exact `persona_source_chapter_high_water`; Agent independently requires that marker to equal its final persisted progress snapshot and injects the summary only as bounded JSON-quoted data. `system_prompt` is never public. Reclaimed turns require the latest validated marker to equal the persisted marker before prompt/provider work. Pre-contract unmarked turns and their unmarked Mid/Long summaries remain exportable but cannot enter online history, prompt composition, count-triggered summaries, semantic retrieval, or completed replay. The [DeepSeek v4 Flash live baseline](./evidence/deepseek-v4-flash-live-baseline.json) predates this world-summary prompt contract; H1 failed and no representative live summary/voice approval exists, so H3 is not complete | H3 |
...
| Local action-type suggestion | Optional, unqualified aid | With a separate Laya (Jev) URL and key configured, an owned self-mode open-world reader can request a type hint for a bounded intent. The browser never changes or submits an action from a hint; existing server checks still validate every selected type and target. This hint path receives only the intent and candidate type descriptions. Uncertainty, failure, or missing configuration leaves manual selection available. Local examples and deterministic checks do not establish representative Chinese accuracy or latency | H4 |
| Novel-specific D20 advanced mode | Optional, unqualified preview | Narrative remains the default. An explicit request lazily generates one immutable template per canonical-model version and exact schema/prompt-version binding, shared across authorized readers of that binding. Existing v1 templates retain their cited-chapter unlock rule. New v2 templates select only fixed server-owned basic capability keys and bounded numeric values from at most 64 stably ordered whole-book `world_rules`; dictionaries provide labels/descriptions and source chapter references are provenance only. V2 requires a ready novel and positive progress; existing narrative, target, event, and hard-rule guards remain. The three-claim ceiling is shared across prompt versions under a canonical-novel lock. Advanced turns retain server-owned hard validation and dice authority. When paired Laya (Jev) settings are present, one bounded semantic classification may select impossible, automatic success, or a difficulty band; absent configuration, uncertainty, and errors fall back to the template DC. Classification and resolution are fenced and frozen in the existing world-turn record before DeepSeek prose; replay reuses the frozen result. No die is shown as evidence for no-check outcomes, and impossible actions follow failed-check mutation clearing. Context is limited to the reviewed allowlist and size cap; it excludes novel full text, scope UUIDs, history, future-event lists, dice, and results. The classifier probability is uncalibrated. V2 source and local validation are separate from paid-provider semantic qualification; current exact-head CI, merge and private deployment evidence is tracked in [issue #422](https://github.com/Wisdoverse/novelworld/issues/422). Full D&D rules, arbitrary free-text guarantees, representative live semantic quality, paid comparison, and human semantic-quality approval are not claimed. See [ADR 0009](./adr/0009-bounded-laya-d20-adjudication.md), [ADR 0010](./adr/0010-versioned-basic-game-rules.md), and the [rules plan](./ADVANCED_RULES_PLAN.md) | H4 |
| Reader-confirmed series worlds | Structural preview; qualification pending | Novel owns private reader series and exact immutable safe source-template snapshots. Laya may suggest a metadata match, but the reader confirms associations and can choose manually. Narrative consumes source rules and target canon separately, freezes the confirmed setting with each bound profile/session, and does not share player attributes, inventory, progress, or checks. Reassociation preserves old bindings and replay. Source and deterministic test coverage are present; required CI, merge, deployment, and representative matching quality are not claimed. Migration 0031 is additive and does not add a release barrier; matching Novel, Narrative, and frontend versions are required. See [ADR 0011](./adr/0011-confirmed-series-worlds.md) | H4 |
...
| No spoilers | Structurally bounded, not guaranteed | Server-owned progress filters lore, committed memory, and the public character-persona read model. The public source-extracted relationship graph is withheld until exact full progress, rechecked after its final relation read, and always `private, no-store`; gradual relationship publication remains open. Agent's Canon v1 goal/relationship projection is also exact-full only because its whole-item descriptions are not chapter slices; partial or rewound requests fail closed before provider I/O. New character facts are content-filtered to explicit witness provenance and stored at the committed session's canonical source high-water; the pre-contract permanent/importance-7/UUID-v4 producer class is retained but quarantined from every direct and semantic prompt path. Character prompt construction omits derived context whose source high-water is missing or later than current progress. World endpoints also return a content-free typed conflict while progress is behind. The supported managed Docker path drains legacy Narrative, Novel, and Agent processes before the relevant 0021/0024/0025/0030 semantic migrations and refuses adoption/upgrade/marked-restore/rollback paths that would revive a pre-barrier writer or reader. In particular, it cannot restore a pre-0024 Agent or Novel release after 0024, or a pre-0025 Agent release after 0025, and it cannot run an old Novel writer after 0030, without a separately approved compatibility procedure. A legitimate legacy row in a quarantined class can be hidden, and an untrusted model can still produce incorrect text. Canon grounding and world-summary live behavior plus export-wide spoiler safety have not yet been proven under one equivalent progress boundary | H3, H4 |
...
`persona_source_chapter_high_water` plus the exact validated `world_summary`,
and still omits `system_prompt`. A partial or rewound character remains readable,
but a new chat is refused before claim/provider work because the whole-novel
summary is not authorized at that progress. Agent also refuses an old or malformed
Novel response whose summary is missing, empty, longer than 2,000 characters, or
does not carry a marker exactly equal to the final persisted progress snapshot.
Character list/detail responses use `Cache-Control: private, no-store`. The browser
...
2. **Formats:** paste, TXT, EPUB, and PDF are accepted within the limits above.
   File upload supports up to 50 selected files, sent sequentially in requests of
   up to five files and 40 MiB each;
...
7. **Permanent memory:** permanent means exempt from normal compression or
   ...
   Agent processes before the 0021/0024/0025/0030 migrations. Both old Novel
   and Narrative writers are therefore stopped before the 0030 key change. An installation running
   ...
   adoption requires a target containing all four required migrations
   (0021/0024/0025/0030); upgrade, marked restore, and rollback fail closed
   before crossing any barrier backwards. On the supported managed Docker path,
   a post-0024 database must not run a pre-0024 Agent or Novel release, a
   post-0025 database must not run a pre-0025 Agent release, and a post-0030
   database must not run the old Novel writer. Crossing those tooling-enforced
   barriers requires a separately approved compatibility procedure. Experimental desktop
