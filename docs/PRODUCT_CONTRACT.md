...
| Local action-type suggestion | Optional, unqualified aid | With a separate Laya URL and key configured, an owned self-mode open-world reader can request a type hint for a bounded intent. The browser never changes or submits an action from a hint; existing server checks still validate every selected type and target. Laya receives only the intent and candidate type descriptions. Model uncertainty, failure, or missing configuration leaves the manual form available. Local synthetic examples and deterministic tests do not establish representative Chinese accuracy or latency | H4 |
| Novel-specific D20 advanced mode | Structurally verified, opt-in preview | The default remains narrative mode. An explicit request lazily generates one source-bound attribute/DC template per canonical novel model and shares it across authorized readers; the exact immutable template stays hidden until every cited chapter is unlocked, with a content-free `422 game_rules_unavailable_at_progress` through both services and Chinese reader guidance; generation is leased, fenced, and capped at three persisted claims with one logical generation call per claim (the shared transport retry policy may replay that same request). Advanced turns use the fixed action-type attribute/DC mapping, persist a server-owned D20 result before provider prose, and replay it through the world-turn ledger. Structural target/hard-rule checks remain authoritative. Laya is an optional type hint, not a semantic referee; no Jev per-action judge or full D&D rules engine is connected. Arbitrary free-text reasonableness and representative live template quality are not qualified. The [rules/evaluation plan](./ADVANCED_RULES_PLAN.md) records current responsibilities and the conditional direct-DeepSeek-versus-Jev experiment; model confidence never substitutes for dice success | H4 |
