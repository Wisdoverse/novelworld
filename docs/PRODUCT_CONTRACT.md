...
| Retry/restart without duplicate committed chat or world turns | Structurally verified | Import resume remains missing; live dependency and long-window recovery evidence is incomplete | H1, H5 |
...
4. **Source retention and reprocessing:** original uploaded bytes are retained
   only when S3 is enabled. Without it, extraction is transient. Retention does
   not yet provide restart-safe import resume.
