...
7. **Permanent memory:** permanent means exempt from normal compression or
   ...
   Agent processes before the 0021/0024 migrations. An installation running
   release tooling that predates either barrier first needs a control-only
   release with the new script, followed by the migration release. New-script
   adoption requires a target containing both migrations; upgrade, marked
   restore, and rollback fail closed before crossing either barrier backwards.
   On the supported managed Docker path, a post-0024 database must not run a
   pre-0024 Agent or Novel release; crossing that tooling-enforced barrier
