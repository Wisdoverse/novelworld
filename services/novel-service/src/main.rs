...
async fn trace_middleware(request: Request, next: Next) -> Response {
    ...
    let span = tracing::info_span!(target: "novelworld_context", "service", service = "novel-service", trace_id = %trace_id);
    async {
...
async fn run_body() -> Result<()> {
    ...
    let service_span = tracing::info_span!(target: "novelworld_context", "service", service = "novel-service", trace_id = "");
    async move {
