...
describe('ShelfPage contracts', () => {
  ...
  it('uses safe guidance for known and unknown import failures and offers the matching action', () => {
    mocks.novels = [
      ...
      { id: 'canon-exhausted', title: '模型校验失败', status: 'error', parse_error: 'AI story model response could not be validated; re-upload the source', total_chapters: 0, updated_at: '2026-01-01T00:00:00Z' },
      { id: 'provider-balance', title: '模型余额不足', status: 'error', parse_error: 'AI provider balance is unavailable; contact the site administrator, then re-upload the source', total_chapters: 0, updated_at: '2026-01-01T00:00:00Z' },
    ...
    expect(screen.getByText('解析失败：故事模型分析结果无法验证，重试次数已用尽，请重新导入原始文件。')).toBeTruthy();
    expect(screen.getByText('解析失败：模型服务余额不足且重试次数已用尽，请联系站点管理员处理后重新导入原始文件。')).toBeTruthy();
    ...
    expect(reimports).toHaveLength(4);
